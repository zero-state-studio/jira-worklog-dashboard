"""
PostgreSQL-based permanent storage for JIRA worklog data.
...
class WorklogStorage:
    """Async PostgreSQL storage for JIRA worklog data - permanent storage."""
...
def get_storage() -> WorklogStorage:
    ...
    global _storage
    if _storage is None:
        # Read database path from environment (for Railway/Docker deployment)
        db_path = os.getenv("DATABASE_PATH", "worklog_storage.db")
        _storage = WorklogStorage(db_path=db_path)
    return _storage
