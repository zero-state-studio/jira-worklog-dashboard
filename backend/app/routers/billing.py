...
async def list_clients():
    ...
    storage = get_storage()
    clients = await storage.get_all_billing_clients()
    return clients


@router.post("/clients")
async def create_client(data: BillingClientCreate):
    ...
    try:
        ...
        raise
    return await storage.get_billing_client(client_id)


@router.put("/clients/{client_id}")
async def update_client(client_id: int, data: BillingClientUpdate):
    """Update a billing client."""
    storage = get_storage()
    updates = data.model_dump(exclude_none=True)
    if not updates:
        raise HTTPException(status_code=400, detail="No fields to update")
    updated = await storage.update_billing_client(client_id, **updates)
    if not updated:
...
async def delete_client(client_id: int):
    """Delete a billing client and all related data."""
    storage = get_storage()
    deleted = await storage.delete_billing_client(client_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Client not found")
    return {"status": "ok"}
...
@router.get("/projects")
async def list_projects(client_id: int = Query(None)):
    """List billing projects, optionally filtered by client."""
    storage = get_storage()
    if client_id:
        return await storage.get_billing_projects_by_client(client_id)
    return await storage.get_all_billing_projects()


@router.post("/projects")
async def create_project(data: BillingProjectCreate):
    """Create a new billing project."""
    storage = get_storage()
    # Verify client exists
    client = await storage.get_billing_client(data.client_id)
    if not client:
        raise HTTPException(status_code=404, detail="Client not found")
    ...
    return await storage.get_billing_project(project_id)


@router.put("/projects/{project_id}")
async def update_project(project_id: int, data: BillingProjectUpdate):
    """Update a billing project."""
    storage = get_storage()
    updates = data.model_dump(exclude_none=True)
    if not updates:
        raise HTTPException(status_code=400, detail="No fields to update")
    updated = await storage.update_billing_project(project_id, **updates)
    if not updated:
...
async def delete_project(project_id: int):
    """Delete a billing project."""
    storage = get_storage()
    deleted = await storage.delete_billing_project(project_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Project not found")
    return {"status": "ok"}
...
async def add_mapping(project_id: int, data: BillingProjectMappingCreate):
    """Add a JIRA project mapping to a billing project."""
    storage = get_storage()
    project = await storage.get_billing_project(project_id)
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    try:
...
async def remove_mapping(project_id: int, mapping_id: int):  # noqa: project_id from URL path
    """Remove a JIRA project mapping."""
    storage = get_storage()
    deleted = await storage.delete_billing_project_mapping(mapping_id)
    if not deleted:
...
@router.get("/rates")
async def list_rates(billing_project_id: int = Query(...)):
    """List billing rates for a project."""
    storage = get_storage()
    return await storage.get_billing_rates(billing_project_id)


@router.post("/rates")
async def create_rate(data: BillingRateCreate):
    """Create a billing rate override."""
    storage = get_storage()
...
async def delete_rate(rate_id: int):
    ...
    storage = get_storage()
    deleted = await storage.delete_billing_rate(rate_id)
    if not deleted:
...
async def classify_worklog(data: BillingClassificationCreate):
    """Classify a worklog as billable/non-billable."""
    storage = get_storage()
...
async def bulk_classify_worklogs(data: BillingClassificationBulk):
    """Bulk classify worklogs as billable/non-billable."""
    storage = get_storage()
...
async def billing_preview(
    ...
    """Get a live billing preview for a client in a period."""
    try:
...
async def create_invoice(data: InvoiceCreate):
    """Create an invoice from billing preview (snapshot)."""
    storage = get_storage()
    ...
    return await storage.get_invoice(invoice_id)
...
async def list_invoices(
    ...
    storage = get_storage()
    invoices = await storage.get_invoices(client_id=client_id, status=status)
...
async def get_invoice(invoice_id: int):
    ...
    storage = get_storage()
    invoice = await storage.get_invoice(invoice_id)
    if not invoice:
...
async def issue_invoice(invoice_id: int):
    ...
    await storage.update_invoice_status(invoice_id, "ISSUED")
    return await storage.get_invoice(invoice_id)


@router.post("/invoices/{invoice_id}/void")
async def void_invoice(invoice_id: int):
    ...
    await storage.update_invoice_status(invoice_id, "VOID")
    return await storage.get_invoice(invoice_id)
...
async def delete_invoice(invoice_id: int):
    ...
    storage = get_storage()
    deleted = await storage.delete_invoice(invoice_id)
    if not deleted:
...
async def export_invoice_excel(invoice_id: int):
    """Export an invoice as an Excel file."""
    storage = get_storage()
    invoice = await storage.get_invoice(invoice_id)
    if not invoice:
        raise HTTPException(status_code=404, detail="Invoice not found")
